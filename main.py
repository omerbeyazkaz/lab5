...
user_number=int(input("Please enter a number between 3-7 : "))
while user_number < 3 or user_number > 7 
  User_number=int(input("İnvalid input please enter between 3-7 : " ))
  
chosen_word=random.sample(words,user_number)
passGen = "".join(chosen_words)
passGen = passGen [:: -1]


#Q2
initialpass = passGen(chosen_words)
print("İnitial password is:--- " + initialpass)

def rep_with_upper(word):
    random_letter = random.choice(word)
    uppercase_letter = random_letter.upper()
    word = word.replace(random_letter,uppercase_letter)
    return word
...
def swap_letters(word):
    ...
    lasttwo = word [-2:]
    middle = word[2:-2]
    lastword = lasttwo + middle + firsttwo
    return lastword

newpassword =  rep_with_upper(str(initialpass))
print("New password is:--- " + newpassword)

finalpassword = swap_letters(str(newpassword))
print("Final password is:--- " + finalpassword)

#Q3

def search_letter(source,keyletter):
    if(keyletter in source):
        print(True)
        return True
    else:
        print(False)
...
search_letter(str(finalpassword),"H")
